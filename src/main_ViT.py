...
def load_model(architecture, model):
  ...
  if model == "ViT":
     model = MyViT((3, 224, 224), n_patches=16, n_blocks=12, hidden_d=768, n_heads=12, out_d=num_classes).to(device)
  return model
...
model_type = "ViT"
architecture = 'ViT'
hyperparams = Hyperparams(Path(base_path) / "data/train_conf.toml", str("5th_channel"), str(architecture))
